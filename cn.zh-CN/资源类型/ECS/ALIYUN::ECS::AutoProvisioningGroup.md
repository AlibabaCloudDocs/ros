...
|LaunchTemplateId|String|否|否|弹性供应组关联的实例启动模板的ID。您可以调用[DescribeLaunchTemplates](/cn.zh-CN/API参考/启动模板/DescribeLaunchTemplates.md)查询可用的实例启动模板。|同时指定LaunchTemplateId和LaunchConfiguration时，优先使用启动模板。|
