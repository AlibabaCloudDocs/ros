...
单位：GB。|
|DBInstanceClass|String|是|否|实例规格。|更多信息，请参见[实例规格表](/cn.zh-CN/产品简介/实例规格表.md)。|
...
|SecurityIPArray|String|否|否|所有可以访问创建或克隆实例的IP地址。|IP地址以半角逗号（,）隔开，不可重复，最多支持1000个。
...
-   4.0
-   4.2 |
|StorageEngine|String|否|否|存储引擎。|关于存储引擎与版本选择的约束详情，请参见[版本及存储引擎](/cn.zh-CN/产品简介/版本及存储引擎.md)。 取值：
