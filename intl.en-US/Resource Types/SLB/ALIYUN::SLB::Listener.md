...
-   black: specifies the ACL as a blacklist. All requests from the IP addresses or CIDR blocks in the ACL are rejected. Blacklists are applicable to scenarios where you want an application to be denied access only from specific IP addresses. If a blacklist is enabled but the corresponding ACL does not contain IP addresses, the SLB listener forwards all requests. This parameter is required when the AclStatus parameter is set to on. |
