...
|BackendServerWeight|Integer|No|No|The weight that is assigned to the ECS instance in the Server Load Balancer \(SLB\) instance.|Valid values: 0 to 100. Default value: 100. |
...
|ImageId|String|No|Yes|The ID of the image that is used to start the created ECS instances. You can use a public image, a custom image, or an Alibaba Cloud Marketplace image.|You can specify a partial public image ID instead of providing the complete ID. Examples:
