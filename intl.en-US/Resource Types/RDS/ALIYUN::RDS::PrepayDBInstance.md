...
|PeriodType|String|Yes|No|The unit of the billing cycle.|Default value: Month. Valid values:
...
|PrivateIpAddress|String|No|No|The private IP address in the CIDR block of the specified VSwitch.|If you do not specify this parameter, the system assigns a private IP address.|
...
|EngineVersion|String|Yes|No|The version of the database engine that the instance runs.|-   Valid values when Engine is set to MySQL: 5.5, 5.6, 5.7, and 8.0.
...
|PreferredBackupPeriod|List|No|No|The list of one or more backup periods.|Valid values: -   Monday
-   Tuesday
-   Wednesday
-   Thursday
-   Friday
...
Unit: GB. This value must be in 5 GB increments.|
...
|Period|Number|Yes|No|The subscription period of the instance.|-   Valid values when PeriodType is set to Month: 1 to 9.
-   Valid values when PeriodType is set to Year: 1 to 3. |
|MasterUserPassword|String|No|No|The password of the database account.|The password must be 8 to 32 characters in length. The password must contain at least three of the following character types: uppercase letters, lowercase letters, digits, and special characters. Special characters include```
! @#$&amp;%^*()_+-=
``` |
...
If you create an instance with local SSDs, you can name the time zone. |
