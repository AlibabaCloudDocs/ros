...
**Note:** The `ALIYUN::OOS::Parameter::Value` and `ALIYUN::OOS::SecretParameter::Value` parameters cannot be verified by the AllowedPattern parameter. |
...
|AssociationProperty|No|The associated resource property. If you specify this parameter property, ROS verifies whether the specified parameter value is valid and provides a list of valid values based on the associated resource property. Valid values:
...
For example, if you set the AssociationProperty parameter property to `ALIYUN::ECS::Instance::ImageId`, the ROS console verifies whether the specified image ID is valid and lists other valid values in a drop-down list. For more information, see [Example 2: the UserName, PassWord, and ImageId parameters](#section_i5w_x3v_kfb). |
...
For more information, see [Example 3: the VSwitchId parameter that uses the AssociationPropertyMetadata parameter property](#section_dbf_br8_mh1).|
...
## Example 1: the InstanceType parameter that is used to create a stack in a web application

For example, you want to create a stack in a web application, and the stack needs to contain one Server Load Balancer \(SLB\) instance, two ECS instances, and one ApsaraDB RDS instance. If the web application is under heavy workload, you can select an ECS instance that has high specifications when you create the stack. If the web application is not under heavy workload, you can select an ECS instance that has lower specifications. The following example shows how to define the InstanceType parameter for an ECS instance:
...
## Example 2: the UserName, PassWord, and ImageId parameters
...
-   PassWord: a String-type parameter that does not have a default value. If you set the NoEcho parameter property to `true`, the GetStack operation does not return parameter values. The parameter value must be 1 to 41 characters in length and can contain uppercase letters, lowercase letters, and digits.
...
## Example 3: the VSwitchId parameter that uses the AssociationPropertyMetadata parameter property
