# Use Ansible ali\_ros\_stack to deploy an LNMP environment

This topic describes how to use the Ansible ali\_ros\_stack module to call the ROS API operations for deploying an LNMP \(Linux, NGINX, MySQL, and PHP\) environment.

Ansible is installed on Linux with pip3 and configured.

1.  Create a file named create\_lnmp.yml and open it in the visual editor.

    ```
    vi create_lnmp.yml
    ```

2.  In edit mode, copy the following playbook code to the create\_lnmp.yml file:

    **Note:** For more information about the ali\_ros\_stack module parameters, see [Parameters](/intl.en-US/Common Tools/Plug-in tools/Ansible/Overview.md).
...
4.  Create a file named create\_lnmp\_instance.json and open it in the visual editor.

    ```
    vi create_lnmp_instance.json
    ```

5.  In edit mode, copy the following playbook code to the create\_lnmp\_instance.json file:
