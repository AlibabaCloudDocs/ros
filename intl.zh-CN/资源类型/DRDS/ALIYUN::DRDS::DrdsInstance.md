...
-   drds.sn1.16c32g
-   drds.sn1.32c64g |
|Specification|String|是|否|实例规格。例如，drds.sn1.4c8g.8C16G由DRDS实例系列（drds.sn1.4c8g）加上具体的实例规格（8C16G）组成。DRDS实例规格取值范围请参见：[分布式关系型数据库服务规格和定价](https://www.aliyun.com/price/product?spm=a2c4g.11186623.2.11.244e6e005na1mr#/drds/detail)。|无|
|PayType|String|是|否|付费类型

|取值： -   drdsPost
-   drdsPre |
|ZoneId|String|是|否|可用区。一个可用区属于某个地域，如杭州可用区A（cn-hangzhou-a）属于地域杭州（cn-hangzhou）。|无|
...
|IsAutoRenew|Boolean|否|否|是否自动续费|如果按月购买则自动续费一个月，如果按年购买则自动续费一年。付费类型是drdsPre时此参数生效。|
