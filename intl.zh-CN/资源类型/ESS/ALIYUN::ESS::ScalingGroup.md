...
|ScalingGroupName|String|否|是|伸缩组的显示名称。|长度为2~64个字符。以数字、英文字母或汉字开头，可包含英文字母、汉字、数字、下划线（\_）、短划线（-）和半角句号（.）。 同一账号同一地域内名称唯一。
...
|Tags|List|否|是|标签。|最多支持添加20个标签。更多信息，请参见[Tags属性](#section_gwz_9kl_6az)。 |
...
更多示例，请参见创建伸缩组、创建伸缩规则、创建监控项报警任务、启动报警任务、为伸缩组创建生命周期挂钩和创建定时任务的组合示例：[JSON示例](https://github.com/aliyun/ros-templates/tree/master/ResourceTemplates/ESS/JSON/ScalingGroup.json)和[YAML示例](https://github.com/aliyun/ros-templates/tree/master/ResourceTemplates/ESS/YAML/ScalingGroup.yml)。
