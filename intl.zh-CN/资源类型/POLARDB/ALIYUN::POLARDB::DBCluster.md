...
|DBClusterDescription|String|否|是|集群名称。|长度为2~256个字符。以英文字母和汉字开头，不能以`http://`或`https://`开头。可包含英文字母、汉字、数字、英文逗号（，）和短划线（-） 。|
|DBType|String|是|否|数据库引擎类型。|取值：-   MySQL
-   PostgreSQL
-   Oracle |
|ClusterNetworkType|String|否|否|网络类型。|当前仅支持专有网络，取值固定为VPC。|
|RenewalStatus|String|否|是|自动续费状态。|取值：-   AutoRenewal：自动续费。
-   Normal（默认值）：手动续费。
-   NotRenewal：不再续费。

**说明：** 设置为NotRenewal后，系统不再发送到期提醒，只在到期前第三天发送不续费提醒。 |
|AutoRenewPeriod|Integer|否|是|设置实例自动续费时长。|取值：-   1（默认值）
-   2
-   3
-   6
-   12
...
单位：月。 |
|ZoneId|String|否|否|可用区ID。|可调用[DescribeRegions](/intl.zh-CN/API参考/其他接口/DescribeRegions.md)接口进行查看。|
...
|DBVersion|String|是|否|数据库版本号。|取值： -   当DBType=MySQL时：5.6、8.0。
-   当DBType=PostgreSQL时：11。
-   当DBType=Oracle时：11。 |
...
|DBNodeClass|String|是|否|节点规格。|详情请参见[规格与定价](/intl.zh-CN/产品定价/规格与定价.md)。 |
|VSwitchId|String|否|否|交换机ID。|无|
|SecurityIPList|String|否|否|白名单。|无|
|CloneDataPoint|String|否|否|克隆数据的时间节点。|取值：-   LATEST（默认值）：最新时间点的数据。

**说明：** 如果CreationOption取值为CloneFromRDS，该参数的取值为LATEST。
...
-   Prepaid：预付费，包年包月。 |
|CreationCategory|String|否|否|集群系列。|取值为Normal（标准版）。|
...
**说明：** 当DBType取值为PostgreSQL或DBType取值为Oracle时，该参数有意义。 |
