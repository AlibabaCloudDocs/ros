...
2.  调用[GetResourceTypeTemplate](/intl.zh-CN/API参考/资源相关接口/GetResourceTypeTemplate.md)接口查询ALIYUN::VPC::EIP的模板（TemplateBody）。

    1.  搜索GetResourceTypeTemplate接口。

    2.  将**ResourceType**设置为ALIYUN::VPC::EIP，然后单击**发起调用**。

        **说明：** 更多资源类型，请参见[资源类型索引](/intl.zh-CN/资源类型/资源类型索引.md)。
...
3.  调用[GetTemplateSummary](/intl.zh-CN/API参考/模板相关接口/GetTemplateSummary.md)接口获取模板信息，查询资源导入标识符。
